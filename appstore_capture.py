...
PASSWORD = "1123581321$$"
OUT = "/app/frontend/appstore-screenshots/raw"
os.makedirs(OUT, exist_ok=True)

# 6.7" iPhone: logical 428x926 * DSR 3 = 1284x2778
...
def wait_root(page, contains=None, timeout=45000):
    page.wait_for_function(
        "() => { const r=document.querySelector('#root'); return r && r.innerText && r.innerText.length>0; }",
        timeout=timeout,
    )
    if contains:
        page.wait_for_function(
            "(t) => { const r=document.querySelector('#root'); return r && r.innerText && r.innerText.includes(t); }",
...
with sync_playwright() as p:
    ...
    ctx = browser.new_context(viewport=VIEWPORT, device_scale_factor=DSR,
    ...
    print("  landed:", page.inner_text('#root')[:60].replace('\n', ' '))
